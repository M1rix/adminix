...
import uz.apextech.adminix.repository.OrderRepository;
import uz.apextech.adminix.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link uz.apextech.adminix.domain.Order}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Order> result = orderRepository
            ...
            .map(existingOrder -> {
                if (order.getWidth() != null) {
                    existingOrder.setWidth(order.getWidth());
                }
                if (order.getHeight() != null) {
                    existingOrder.setHeight(order.getHeight());
                }
