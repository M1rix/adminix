...
function sidebarColor(e) {
  ...
  e.classList.contains('active') ? e.classList.remove('active') : e.classList.add('active'),
    ...
    document.querySelector('#sidenavCard') &&
      ...
      (s = document.querySelector('#sidenavCard+.btn+.btn')).removeAttribute('class'),
      s.classList.add(...e));
}
...
function navbarColorOnResize() {
  1200 < window.innerWidth
    ? referenceButtons.classList.contains('active') && 'bg-transparent' === referenceButtons.getAttribute('data-class')
      ? sidenav.classList.remove('bg-white')
      : body.classList.contains('dark-version') || sidenav.classList.add('bg-white')
...
function darkMode(e) {
  ...
  if (e.getAttribute('checked')) {
    t.classList.remove('dark-version'),
      ...
      p.includes('logo-ct.png') && ((q = p.replace('logo-ct', 'logo-ct-dark')), (y.src = q));
    for (A = 0; A < k.length; A++)
      k[A].classList.contains('text-dark') && (k[A].classList.add('text-white'), k[A].classList.remove('text-dark'));
    for (A = 0; A < S.length; A++)
      S[A].classList.contains('text-white') && (S[A].classList.remove('text-white'), S[A].classList.add('text-dark'));
    ...
    for (A = 0; A < l.length; A++)
      l[A].classList.contains('text-white') && (l[A].classList.remove('text-white'), l[A].classList.add('text-dark'));
    ...
    for (A = 0; A < d.length; A++)
      d[A].classList.contains('text-white') && (d[A].classList.remove('text-white'), d[A].classList.add('text-dark'));
    for (A = 0; A < v.length; A++)
      v[A].classList.contains('text-white') &&
        (v[A].classList.remove('text-white'), v[A].classList.remove('opacity-8'), v[A].classList.add('text-dark'));
    for (A = 0; A < m.length; A++)
      m[A].classList.contains('bg-gray-600') && (m[A].classList.remove('bg-gray-600'), m[A].classList.add('bg-gray-100'));
    for (A = 0; A < x.length; A++) x[A].hasAttribute('fill') && x[A].setAttribute('fill', '#252f40');
    for (A = 0; A < h.length; A++)
      h[A].closest('.card.bg-gradient-dark') || (h[A].classList.remove('text-white'), h[A].classList.add('text-dark'));
    ...
    for (var A = 0; A < S.length; A++)
      S[A].classList.contains('text-dark') && (S[A].classList.remove('text-dark'), S[A].classList.add('text-white'));
    ...
    for (var A = 0; A < l.length; A++)
      l[A].classList.contains('text-dark') && (l[A].classList.remove('text-dark'), l[A].classList.add('text-white'));
    for (var A = 0; A < r.length; A++)
      r[A].classList.contains('text-dark') && (r[A].classList.remove('text-dark'), r[A].classList.add('text-white'));
    for (var A = 0; A < c.length; A++)
      c[A].classList.contains('text-dark') && (c[A].classList.remove('text-dark'), c[A].classList.add('text-white'));
    for (var A = 0; A < u.length; A++)
      u[A].classList.contains('text-dark') && (u[A].classList.remove('text-dark'), u[A].classList.add('text-white'));
    for (var A = 0; A < v.length; A++)
      v[A].classList.contains('text-secondary') &&
        (v[A].classList.remove('text-secondary'), v[A].classList.add('text-white'), v[A].classList.add('opacity-8'));
    for (var A = 0; A < g.length; A++)
      g[A].classList.contains('bg-gray-100') && (g[A].classList.remove('bg-gray-100'), g[A].classList.add('bg-gray-600'));
        ...
        i.querySelector('li:nth-child(' + a + ') .nav-link').onclick = function () {
          ...
          if (i.classList.contains('flex-column')) {
            ...
            (l.style.transform = 'translate3d(' + e + 'px, 0px, 0px)'),
              (l.style.width = i.querySelector('li:nth-child(' + a + ')').offsetWidth + 'px');
          }
        };
      }
    });
      ...
      if (i) {
        ...
        if (s.classList.contains('flex-column')) {
          ...
          (a.style.transform = 'translate3d(0px,' + t + 'px, 0px)'),
            (a.style.width = s.querySelector('li:nth-child(' + n + ')').offsetWidth + 'px'),
            (a.style.height = s.querySelector('li:nth-child(' + l + ')').offsetHeight);
        } else {
